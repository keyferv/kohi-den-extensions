...
abstract class AnikotoTheme(
    ...
    internal val m3u8ServerManager by lazy { M3u8ServerManager() }
    ...
    val cacheControl by lazy { CacheControl.Builder().maxAge(1, TimeUnit.HOURS).build() }

    private val excludedHosts: Set<String>
        get() = preferences.getStringSet(PREF_HOSTER_EXCLUDE_KEY, null) ?: emptySet()

    private val excludedTypes: Set<String>
        get() = preferences.getStringSet(PREF_TYPE_EXCLUDE_KEY, null) ?: emptySet()
    ...
    private val getTitleLang: String
        get() = preferences.getString(PREF_TITLE_LANG_KEY, PREF_TITLE_LANG_DEFAULT)!!
    ...
    protected val prefServer: String
        get() = preferences.getString(PREF_SERVER_KEY, hosterNames.firstOrNull() ?: "")!!

    protected val prefType: String
        get() = preferences.getString(PREF_TYPE_KEY, PREF_TYPE_DEFAULT)!!

    protected val scorePosition: String
        get() = preferences.getString(PREF_SCORE_POSITION_KEY, PREF_SCORE_POSITION_DEFAULT)!!

    // ============================== Preferences ===========================

    private fun clearOldPrefs(prefs: SharedPreferences): SharedPreferences {
        try {
            val domain = (prefs.getString(PREF_DOMAIN_KEY, defaultBaseUrl) ?: defaultBaseUrl).removePrefix("https://")
            val invalidDomain = domain !in domainEntries

            val validServers = (
                (prefs.getStringSet(PREF_DISCOVERED_HTML_SERVERS_KEY, null) ?: emptySet()) +
                    (prefs.getStringSet(PREF_DISCOVERED_MAPPER_SERVERS_KEY, null) ?: emptySet())
                ).takeIf { it.isNotEmpty() } ?: hosterNames.toSet()

            val oldHosterSelection = prefs.getStringSet("hoster_selection", null)?.toSet()
            if (oldHosterSelection != null) {
                ...
                prefs.edit().putStringSet(PREF_HOSTER_EXCLUDE_KEY, newExclusion).remove("hoster_selection").apply()
            }
            val currentExcludedHosts = prefs.getStringSet(PREF_HOSTER_EXCLUDE_KEY, null)?.toSet() ?: emptySet()
            val invalidHosters = currentExcludedHosts.any { it !in validServers && it !in hosterNames }

            val validTypes = (
                prefs.getStringSet(PREF_DISCOVERED_TYPES_KEY, null)
                    ?.takeIf { it.isNotEmpty() }
                    ?.toSet()
                    ?: seedTypes
                )

            val oldTypeSelection = prefs.getStringSet("type_selection", null)?.toSet()
            if (oldTypeSelection != null) {
                ...
                prefs.edit().putStringSet(PREF_TYPE_EXCLUDE_KEY, newExclusion).remove("type_selection").apply()
            }
            val currentExcludedTypes = prefs.getStringSet(PREF_TYPE_EXCLUDE_KEY, null)?.toSet() ?: emptySet()
            val invalidTypes = currentExcludedTypes.any { it !in validTypes }

            val savedPrefType = prefs.getString(PREF_TYPE_KEY, PREF_TYPE_DEFAULT) ?: PREF_TYPE_DEFAULT
            val invalidPrefType = savedPrefType !in validTypes

            val savedPrefServer = prefs.getString(PREF_SERVER_KEY, null)
