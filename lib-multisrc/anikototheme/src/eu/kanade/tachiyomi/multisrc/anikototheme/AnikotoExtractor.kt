...
class AnikotoExtractor(private val theme: AnikotoTheme) {
    ...
    suspend fun extractVideos(document: Document, episode: SEpisode, epUrl: String): List<Video> {
        ...
        return serverData.sequentialCatchingFlatMap { server ->
    ...
    private suspend fun proxyThroughM3u8Server(originalUrl: String): String? = try {
    ...
    private suspend inline fun <T, R> List<T>.sequentialCatchingFlatMap(crossinline transform: suspend (T) -> List<R>): List<R> =
        flatMap {
            try {
                transform(it)
