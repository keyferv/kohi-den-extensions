...
abstract class WcoTheme(
    ...
    override fun popularAnimeParse(response: Response): AnimesPage {
        val document = response.asJsoup()
        return document.select(popularAnimeSelector())
            .mapNotNull { element ->
            ...
            .ifEmpty {
                listOf(
                    SEpisode.create().apply {
                        ...
                        val (name, episodeNumber) = episodeTitleFromElement(title)
                        this.name = name
    ...
    override fun episodeFromElement(element: Element): SEpisode {
        val anchor = if (element.tagName() == "a") {
            ...
            element.selectFirst("a") ?: throw IllegalArgumentException("Episode element has no anchor")
        }
    ...
    open fun iframeExtractor(document: Document): List<Video> {
        val iframes = document.select("iframe")
        if (iframes.isEmpty()) {
            return runCatching { iframeOldExtractor(document) }.getOrElse { emptyList() }
        }
    ...
    open suspend fun iframeParse(iframeLink: String): List<Video> {
        return if (iframeLink.contains("embed.wcostream")) {
            ...
            val getVideoLinkScript =
                iframeSoup.selectFirst("script:containsData(getJSON)")?.data() ?: return emptyList()
            val getVideoLink =
                getVideoLinkScript.substringAfter("$.getJSON(\"", missingDelimiterValue = "").substringBefore("\"")
            if (getVideoLink.isBlank()) return emptyList()
