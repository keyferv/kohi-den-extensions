...
abstract class AnimeKaiTheme(
    ...
    override var baseUrl: String
        ...
        set(value) { preferences.edit().putString(PREF_DOMAIN_KEY, value).apply() }

    protected open val rateLimit = 5

    protected var docHeaders: Headers by LazyMutable { headersBuilder().build() }
    ...
    private val cacheControl by lazy { CacheControl.Builder().maxAge(1, TimeUnit.HOURS).build() }

    init {
        clearOldPrefs(preferences)
    }
    ...
    ).awaitSuccess().use { it.parseAs<ResultResponse>() }.result
    ...
    protected open suspend fun encDecEndpoints(enc: String): String {
        ...
        return fetchEncDec({ GET(url, encDecHeaders()) }) {
            parseAs<ResultResponse>()
        }.result
    }

    protected open suspend fun decryptIframeData(encryptedText: String, headers: Headers): String {
        val postBody = buildJsonObject { put("text", encryptedText) }
            .toString()
            .toRequestBody("application/json; charset=utf-8".toMediaType())
        return fetchEncDec({ POST("https://enc-dec.app/api/dec-kai", body = postBody, headers = headers) }) {
            parseAs<IframeResponse>()
        }.result.url
    }

    protected open suspend fun <T> fetchEncDec(requestFactory: () -> Request, parse: Response.() -> T): T {
        var lastError: Exception? = null

        repeat(ENC_DEC_MAX_ATTEMPTS) {
            try {
                return client.newCall(requestFactory()).awaitSuccess().use { it.parse() }
    ...
    private fun clearOldPrefs(prefs: SharedPreferences) {
        val domain = prefs.getString(PREF_DOMAIN_KEY, defaultBaseUrl)!!.removePrefix("https://")
        val hostToggle = prefs.getStringSet(PREF_HOSTER_KEY, hosterNames.toSet())!!
    ...
    protected val useEnglish: Boolean
        get() = getTitleLang == "English"

    protected val getTitleLang: String
        get() = preferences.getString(PREF_TITLE_LANG_KEY, PREF_TITLE_LANG_DEFAULT) ?: PREF_TITLE_LANG_DEFAULT

    protected val prefQuality: String
        get() = preferences.getString(PREF_QUALITY_KEY, PREF_QUALITY_DEFAULT) ?: PREF_QUALITY_DEFAULT

    protected val prefServer: String
        get() = preferences.getString(PREF_SERVER_KEY, hosterNames.first()) ?: hosterNames.first()

    protected val prefType: String
        get() = preferences.getString(PREF_TYPE_KEY, PREF_TYPE_DEFAULT) ?: PREF_TYPE_DEFAULT

    protected val hostToggle: Set<String>
        get() = preferences.getStringSet(PREF_HOSTER_KEY, hosterNames.toSet()) ?: hosterNames.toSet()

    protected val typeToggle: Set<String>
        get() = preferences.getStringSet(PREF_TYPE_TOGGLE_KEY, DEFAULT_TYPES) ?: DEFAULT_TYPES

    protected val scorePosition: String
        get() = preferences.getString(PREF_SCORE_POSITION_KEY, PREF_SCORE_POSITION_DEFAULT) ?: PREF_SCORE_POSITION_DEFAULT
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) {
        ...
        ListPreference(screen.context).apply {
            ...
            entries = PREF_TITLE_LANG_LIST.toTypedArray()
            entryValues = PREF_TITLE_LANG_LIST.toTypedArray()
        ...
        ListPreference(screen.context).apply {
            ...
            entries = TYPES_ENTRIES.toTypedArray()
            entryValues = TYPES_ENTRIES.toTypedArray()
        ...
        MultiSelectListPreference(screen.context).apply {
            ...
            entries = TYPES_ENTRIES.toTypedArray()
            entryValues = TYPES_VALUES.toTypedArray()
    ...
    private class LazyMutable<T>(private val initializer: () -> T) : ReadWriteProperty<Any?, T> {
    ...
    private suspend inline fun <T, R> List<T>.parallelCatchingMapNotNull(crossinline transform: suspend (T) -> R?): List<R> =
        mapNotNull {
            try {
                transform(it)
            } catch (e: CancellationException) {
    ...
    private suspend inline fun <T, R> List<T>.parallelCatchingFlatMap(crossinline transform: suspend (T) -> List<R>): List<R> =
        flatMap {
            try {
                transform(it)
            } catch (e: CancellationException) {
    ...
    companion object {
        ...
        private const val ENC_DEC_MAX_ATTEMPTS = 2
