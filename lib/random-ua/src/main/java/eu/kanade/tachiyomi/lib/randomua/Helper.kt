...
private val client = Injekt.get<NetworkHelper>().client.newBuilder()
    ...
    .apply {
        val index = networkInterceptors().indexOfFirst { it.javaClass.name == "okhttp3.brotli.BrotliInterceptor" }
...
internal fun getRandomUserAgent(
    ...
    val userAgentList = uaResponse.use { it.body.string().parseAs<UserAgentList>() }
