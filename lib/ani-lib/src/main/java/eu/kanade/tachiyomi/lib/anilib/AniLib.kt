package eu.kanade.tachiyomi.lib.anilib
...
private inline fun <reified T> String.parseAs(): T = graphQLJson.decodeFromString(this)

private inline fun <reified T> T.toJsonString(): String = graphQLJson.encodeToString(this)

private inline fun <reified T> Response.parseGraphQLAs(): T {
    val bodyString = body.string()
    val envelope = graphQLJson.decodeFromString<GraphQLResponse>(bodyString)
    envelope.errors?.let { if (it.isNotEmpty()) throw GraphQLException(it.joinToString("\n") { it.message }) }
    val data = envelope.data ?: throw IllegalStateException("GraphQL response is missing the 'data' field")
    return graphQLJson.decodeFromJsonElement(data)
}

private fun graphQLPost(url: String, headers: Headers, query: String, variables: JsonObject): Request {
